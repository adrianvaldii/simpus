<?php session_start();
  ...
  if (isset($_POST['submit_pusat'])) {
    $query = "MERGE INTO rekam_medis r USING (SELECT * FROM rekam_medis@to_pusat) p ON (r.id_daftar = p.id_daftar)
              WHEN MATCHED THEN UPDATE SET r.tgl_daftar = p.tgl_daftar, r.anamnesa = p.anamnesa, r.pemeriksaan = p.pemeriksaan, r.diagnosis = p.diagnosis, r.terapi = p.terapi, r.status = p.status, r.id_pasien = p.id_pasien, r.id_pelayanan = p.id_pelayanan, r.id_dokter = p.id_dokter, r.id_perawat = p.id_perawat, r.id_apoteker = p.id_apoteker, r.hasil_lab = p.hasil_lab
  ...
  if (isset($_POST['submit_resepsionis'])) {
    $query = "MERGE INTO rekam_medis p USING (SELECT * FROM rekam_medis@to_resepsionis) r ON (p.id_daftar = r.id_daftar)
              WHEN MATCHED THEN UPDATE SET p.tgl_daftar = r.tgl_daftar, p.anamnesa = r.anamnesa, p.pemeriksaan = r.pemeriksaan, p.diagnosis = r.diagnosis, p.terapi = r.terapi, p.status = r.status, p.id_pasien = r.id_pasien, p.id_pelayanan = r.id_pelayanan, p.id_dokter = r.id_dokter, p.id_perawat = r.id_perawat, p.id_apoteker = r.id_apoteker, p.hasil_lab = r.hasil_lab
  ...
  if (isset($_POST['submit_dokter'])) {
    $query = "MERGE INTO rekam_medis r USING (SELECT * FROM rekam_medis@to_dokter) d ON (r.id_daftar = d.id_daftar)
              WHEN MATCHED THEN UPDATE SET r.tgl_daftar = d.tgl_daftar, r.anamnesa = d.anamnesa, r.pemeriksaan = d.pemeriksaan, r.diagnosis = d.diagnosis, r.terapi = d.terapi, r.status = d.status, r.id_pasien = d.id_pasien, r.id_pelayanan = d.id_pelayanan, r.id_dokter = d.id_dokter, r.id_perawat = d.id_perawat, r.id_apoteker = d.id_apoteker, r.hasil_lab = d.hasil_lab
  ...
  if (isset($_POST['submit_apoteker'])) {
    $query = "MERGE INTO rekam_medis r USING (SELECT * FROM rekam_medis@to_apoteker) a ON (r.id_daftar = a.id_daftar)
              WHEN MATCHED THEN UPDATE SET r.tgl_daftar = a.tgl_daftar, r.anamnesa = a.anamnesa, r.pemeriksaan = a.pemeriksaan, r.diagnosis = a.diagnosis, r.terapi = a.terapi, r.status = a.status, r.id_pasien = a.id_pasien, r.id_pelayanan = a.id_pelayanan, r.id_dokter = a.id_dokter, r.id_perawat = a.id_perawat, r.id_apoteker = a.id_apoteker, r.hasil_lab = a.hasil_lab
      ...
      <div class="row">
        ...
        <div class="col-md-10 content">
          ...
          <div class="row">
            ...
            <div class="col-md-6">
              <fieldset class="sinkron">
                <legend class="sinkron">Sinkronisasi Data Server Resepsionis - Server Dokter</legend>
                <h3>Tekan tombol 'Sinkronisasi' untuk sinkronisasi data</h3>
                <?php
                  if (isset($_POST['submit_dokter'])) {
                    ?><div class="alert alert-info" role="alert"><?php echo $status; ?></div><?php
            ...
            <div class="col-md-6">
              <fieldset class="sinkron">
                <legend class="sinkron">Sinkronisasi Data Server Resepsionis - Server Apoteker</legend>
                <h3>Tekan tombol 'Sinkronisasi' untuk sinkronisasi data</h3>
                <?php
                  if (isset($_POST['submit_apoteker'])) {
                    ?><div class="alert alert-info" role="alert"><?php echo $status; ?></div><?php
